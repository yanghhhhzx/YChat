...
//    public static List<ChannelHandlerContext> channelGroup;
    ...
    protected void channelRead0(ChannelHandlerContext ctx, TextWebSocketFrame msg) throws Exception {
        ...
        ctx.writeAndFlush(new TextWebSocketFrame("服务端接受到消息：" + msg.text()));
...
//            messageToOne.setToOne(message.getSender());//把id改为当前用户
            ...
            ctx.writeAndFlush(new TextWebSocketFrame("转发消息成功"));
            //*****上面代码只是方便测试：给自己发一个短信 *****
        }
    ...
    public static int sendMessageByUserId(MessageToOne messageToOne){
        ...
        if (channels.containsKey(messageToOne.getToOne())){
            ...
            channels.get(messageToOne.getToOne()).writeAndFlush(new TextWebSocketFrame(String.valueOf(message)));
            return 1;
        }
